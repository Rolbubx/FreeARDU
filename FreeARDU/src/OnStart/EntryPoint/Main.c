...
#include "../../Framebuffer/FramebufferApi.h"
...
extern uint32_t upd_mode_entry(uint32_t mhz);
...
#define BUILD_VERSION "Build 1.0.2"
...
void display_splash() {
    ...
    // Demo uGUI
                    ...
                    UG_DrawLine(0, 0, (UG_S16)display_get_width(), (UG_S16)display_get_height(), C_RED);
                    UG_DrawLine(0, (UG_S16)display_get_height(), (UG_S16)display_get_width(), 0, C_RED);
                    display_flush();
                } else {
...
int main() {
    ...
    // Register commands
    command_register("HELP", cmd_help, true);
    command_register("VERSION", cmd_version, true);
    command_register("UPTIME", cmd_uptime, true);
    command_register("CPUINFO", cmd_cpuinfo, true);
    command_register("MEMSTAT", cmd_memstat, true);
    command_register("DATE", cmd_date, true);
    command_register("LS", cmd_ls, true);
    command_register("ECHO", cmd_echo, true);
    command_register("CLEAR", cmd_clear, true);
    command_register("CLR", cmd_clear, true);
    command_register("CRT", cmd_crt_callback, true);
    command_register("CD", cmd_cd_callback, true);
    command_register("RD", cmd_rd_callback, true);
    
    // New commands
    command_register("AND", cmd_and, true);
    command_register("KILL", cmd_kill_proc, true);
    command_register("LST PRC", cmd_list_processes, true);
    command_register("SYS MSTR", cmd_system_start_manager, true);
    command_register("BIOS", cmd_bios_load, true);
    ...
    #if DEBUG_COMMANDS_ENABLED
        command_register("DUMP_MEMORY", cmd_dump_memory, true);
        command_register("MEMTEST", cmd_memtest, true);
        command_register("SCREEN_INFO", cmd_screen_info, true);
        command_register("STRESS_TEST", cmd_stress_test, true);
        command_register("RESTART", cmd_restart, true);
        command_register("HALT", cmd_halt, true);
        command_register("PANIC", cmd_panic, true);
        command_register("CAUSE_FAULT", cmd_cause_fault, true);
    #endif
