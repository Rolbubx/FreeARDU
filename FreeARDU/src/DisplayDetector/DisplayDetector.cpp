...
// Declaration comes from GpioBareMetal.h (extern "C" linkage)
#else
#include <Arduino.h>
#include <Wire.h>
#include <SPI.h>
...
extern "C" ScreenDetectionResult DETECT_SCRN() {
    ...
    ScreenDetectionResult res = makeEmptyResult();
    res.type = SCREEN_SPI_DISPLAY;
    res.width = FREEARDU_DISPLAY_WIDTH;
    res.height = FREEARDU_DISPLAY_HEIGHT;
    res.mosiPin = LPSPI4_MOSI_PIN;
    res.sckPin = LPSPI4_SCK_PIN;
    res.csPin = LPSPI4_CS_PIN;
    res.dcPin = LPSPI4_DC_PIN;
    res.resetPin = LPSPI4_RST_PIN;
    res.canDraw = true;
    return res;
